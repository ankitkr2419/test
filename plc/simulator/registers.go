...
var REGISTERS_EXTRACTION map[string]map[string]map[uint16]uint16 = map[string]map[string]map[uint16]uint16{
	...
	"B": map[string]map[uint16]uint16{
		...
		"D": map[uint16]uint16{
			...
			plc.MODBUS_EXTRACTION["B"]["D"][210]: uint16(roomTemp * 10), // Shaker Temperature present value LH (R)
			...
			plc.MODBUS_EXTRACTION["B"]["D"][224]: uint16(roomTemp * 10), // Shaker temperature present value RH (R)
