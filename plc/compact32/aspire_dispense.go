...
variables: category, cartridgeType string,
  ...
  3. store the source_well distance into sourcePosition variable
  4. store the destination_well distance into destinationPosition variable; goto 9
  5. store the shaker distance into sourcePosition variable
  6. store the destination_well distance into destinationPosition variable; goto 9
  7. store the source_well distance into sourcePosition variable
  8. store the shaker distance into destinationPosition variable
...
func (d *Compact32Deck) AspireDispense(category, cartridgeType string, labwareID, source, destination, aspire_cycles, dispense_cycles int64, asp_height, asp_mix_vol, asp_vol, dis_height, dis_mix_vol, dis_vol, dis_blow float64) (response string, err error) {
	...
	var sourcePosition, destinationPosition, distToTravel, distance float64
	var ok bool
	...
	*/
	...
	// TODO: Check for tip type and use consDistance accordingly
	if distance, ok = consDistance["pickup_extraction_tip_up"]; ok {
		distToTravel = positions[deckAndMotor] - distance
	} else {
		err = fmt.Errorf("pickup_extraction_tip_up doesn't exist for consuamble distances")
		fmt.Println("Error: ", err)
		return "", err
	}
	...
	if err != nil {
		fmt.Println(err)
		return "", fmt.Errorf("There was issue moving Syringe Module Up with tip. Error: %v", err)
	}
