...
// NOTE: timeoutMs has caused us problems, make sure it matches and works correctly
// Also if the Application type is Combined then a separate station_SlaveID needs to be created
const (
	HOLDING_STAGE     = "hold"
	CYCLE_STAGE       = "cycle"
	baudRate          = 9600
...
func NewCompact32DeckDriverB(wsMsgCh chan string, wsErrch chan error, exit chan error, test bool, handler *modbus.RTUClientHandler) plc.Extraction {
	/* Modbus RTU/ASCII */
