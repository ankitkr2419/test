...
func (d *Compact32Deck) Piercing(pi db.Piercing, cartridgeID int64) (response string, err error) {
	...
	var position, cartridgeStart, piercingHeight, tipHeight, distToTravel, deckBase float64
	var ok bool
	...
	if tipHeightInter, ok = tipstubes["piercing_tip"]["height"]; !ok {
		err = fmt.Errorf("piercing_tip doesn't exist for tipstubes")
		fmt.Println("Error: ", err)
		return "", err
	}

	if tipHeight, ok = tipHeightInter.(float64); !ok {
		err = fmt.Errorf("piercing_tip has unknown type")
		fmt.Println("Error: ", err)
		return "", err
	}
	...
	// Calculation below considers syringe module as glued with
	// tip as in (positions[deckAndMotor] + tipHeight )
	// And we pierce this much below the deck base -> piercingHeight
	distToTravel = deckBase - (positions[deckAndMotor] + tipHeight) + piercingHeight
