...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/consumable-distance", authenticateAdmin(createConsumableDistanceHandler(deps), deps)).Methods(http.MethodPost, http.MethodOptions).Headers(versionHeader, v1)
	...
	router.HandleFunc("/piercing/{recipe_id}", authenticateAdmin(createPiercingHandler(deps), deps)).Methods(http.MethodPost)
	...
	router.HandleFunc("/aspire-dispense/{recipe_id}", authenticateAdmin(createAspireDispenseHandler(deps), deps)).Methods(http.MethodPost)
	router.HandleFunc("/aspire-dispense/{id}", authenticate(showAspireDispenseHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/aspire-dispense/{id}", authenticateAdmin(updateAspireDispenseHandler(deps), deps)).Methods(http.MethodPut)
	...
	router.HandleFunc("/shaking/{recipe_id}", authenticateAdmin(createShakingHandler(deps), deps)).Methods(http.MethodPost)
	...
	router.HandleFunc("/attach-detach/{recipe_id}", authenticateAdmin(createAttachDetachHandler(deps), deps)).Methods(http.MethodPost)
	...
	router.HandleFunc("/tip-docking/{recipe_id}", authenticateAdmin(createTipDockHandler(deps), deps)).Methods(http.MethodPost)
	...
	router.HandleFunc("/heating/{recipe_id}", authenticateAdmin(createHeatingHandler(deps), deps)).Methods(http.MethodPost)
	...
	router.HandleFunc("/delay/{recipe_id}", authenticateAdmin(createDelayHandler(deps), deps)).Methods(http.MethodPost)
	...
	router.HandleFunc("/tip-operation/{recipe_id}", authenticateAdmin(createTipOperationHandler(deps), deps)).Methods(http.MethodPost)
