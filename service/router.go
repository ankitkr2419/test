...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/pause/{deck:[A-B]}", authenticate(pauseHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/resume/{deck:[A-B]}", authenticate(resumeHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/abort/{deck:[A-B]}", authenticate(abortHandler(deps), deps)).Methods(http.MethodGet)
	...
	router.HandleFunc("/aspire-dispense/{recipe_id}", authenticate(createAspireDispenseHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/aspire-dispense/{id}", authenticate(showAspireDispenseHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/aspire-dispense/{id}", authenticate(updateAspireDispenseHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	router.HandleFunc("/recipes", authenticate(createRecipeHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/recipes/{id}/{publish:[a-z]*}", authenticate(publishRecipeHandler(deps), deps, admin)).Methods(http.MethodPost)
	router.HandleFunc("/processes", authenticate(createProcessHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/recipe/{id}/processes", authenticate(listProcessesHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/duplicate-process/{process_id}", authenticate(duplicateProcessHandler(deps), deps, admin, engineer)).Methods(http.MethodGet)
	router.HandleFunc("/rearrange-processes/{recipe_id}", authenticate(rearrangeProcessesHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/processes/{id}", authenticate(showProcessHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/processes/{id}", authenticate(deleteProcessHandler(deps), deps, admin, engineer)).Methods(http.MethodDelete)
	router.HandleFunc("/processes/{id}", authenticate(updateProcessHandler(deps), deps, admin)).Methods(http.MethodPut)
	router.HandleFunc("/run/{id}/{deck:[A-B]}", authenticate(runRecipeHandler(deps, false), deps)).Methods(http.MethodGet)
	router.HandleFunc("/step-run/{id}/{deck:[A-B]}", authenticate(runRecipeHandler(deps, true), deps, admin, engineer)).Methods(http.MethodGet)
	router.HandleFunc("/run-next-step/{deck:[A-B]}", authenticate(runNextStepHandler(deps), deps, admin, engineer)).Methods(http.MethodGet)
	router.HandleFunc("/discard-box/cleanup/{deck:[A-B]}", authenticate(discardBoxCleanupHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/restore-deck/{deck:[A-B]}", authenticate(restoreDeckHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/uv/{time}/{deck:[A-B]}", authenticate(uvLightHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/discard-tip-and-home/{discard}/{deck:[A-B]}", authenticate(discardAndHomeHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/shaking/{recipe_id}", authenticate(createShakingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/shaking/{id}", authenticate(showShakingHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/shaking/{id}", authenticate(updateShakingHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	router.HandleFunc("/attach-detach/{recipe_id}", authenticate(createAttachDetachHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/attach-detach/{id}", authenticate(showAttachDetachHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/attach-detach/{id}", authenticate(updateAttachDetachHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	router.HandleFunc("/tip-docking/{recipe_id}", authenticate(createTipDockHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/tip-docking/{id}", authenticate(showTipDockHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/tip-docking/{id}", authenticate(updateTipDockHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	router.HandleFunc("/heating/{recipe_id}", authenticate(createHeatingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/heating/{id}", authenticate(showHeatingHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/heating/{id}", authenticate(updateHeatingHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	router.HandleFunc("/delay/{recipe_id}", authenticate(createDelayHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/delay/{id}", authenticate(showDelayHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/delay/{id}", authenticate(updateDelayHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	router.HandleFunc("/tip-operation/{recipe_id}", authenticate(createTipOperationHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
