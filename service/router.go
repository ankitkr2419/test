...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/recipes/{id}/publish", publishRecipeHandler(deps)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/duplicate-process/{process_id}", authenticate(duplicateProcessHandler(deps), deps)).Methods(http.MethodGet).Headers(versionHeader, v1)
	...
	router.HandleFunc("/processes/{id}", authenticate(updateProcessHandler(deps), deps)).Methods(http.MethodPut).Headers(versionHeader, v1)
	...
	router.HandleFunc("/step-run/{id}/{deck:[A-B]}", authenticate(runRecipeHandler(deps, true), deps)).Methods(http.MethodGet).Headers(versionHeader, v1)
	router.HandleFunc("/run-next-step/{deck:[A-B]}", authenticate(runNextStepHandler(deps), deps)).Methods(http.MethodGet).Headers(versionHeader, v1)
	...
	router.HandleFunc("/shaking", authenticate(createShakingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/attach-detach", authenticate(createAttachDetachHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/tip-docking", authenticate(createTipDockHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/heating", authenticate(createHeatingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/delay", authenticate(createDelayHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/tip-operation", authenticate(createTipOperationHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/piercing", authenticate(createPiercingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	...
	router.HandleFunc("/aspireDispense", authenticate(createAspireDispenseHandler(deps), deps, admin, engineer)).Methods(http.MethodPost).Headers(versionHeader, v1)
	router.HandleFunc("/aspireDispense/{id}", authenticate(showAspireDispenseHandler(deps), deps)).Methods(http.MethodGet).Headers(versionHeader, v1)
	router.HandleFunc("/aspireDispense/{id}", authenticate(updateAspireDispenseHandler(deps), deps, admin, engineer)).Methods(http.MethodPut).Headers(versionHeader, v1)
