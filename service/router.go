...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/users/login/{deck:[A-B]?}", validateUserHandler(deps)).Methods(http.MethodPost, http.MethodOptions).Headers(versionHeader, v1)
	router.HandleFunc("/motor", createMotorHandler(deps)).Methods(http.MethodPost, http.MethodOptions).Headers(versionHeader, v1)
	router.HandleFunc("/consumabledistance", authenticate(createConsumableDistanceHandler(deps), deps, engineer)).Methods(http.MethodPost, http.MethodOptions).Headers(versionHeader, v1)
	...
	router.HandleFunc("/piercing", authenticate(createPiercingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/aspireDispense", authenticate(createAspireDispenseHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	router.HandleFunc("/aspireDispense/{id}", authenticate(showAspireDispenseHandler(deps), deps)).Methods(http.MethodGet)
	router.HandleFunc("/aspireDispense/{id}", authenticate(updateAspireDispenseHandler(deps), deps, admin, engineer)).Methods(http.MethodPut)
	...
	router.HandleFunc("/shaking", authenticate(createShakingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/attach-detach", authenticate(createAttachDetachHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/tip-docking", authenticate(createTipDockHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/heating", authenticate(createHeatingHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/delay", authenticate(createDelayHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
	...
	router.HandleFunc("/tip-operation", authenticate(createTipOperationHandler(deps), deps, admin, engineer)).Methods(http.MethodPost)
