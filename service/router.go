...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/target/{id:[0-9]+}", updateTargetHandler(deps)).Methods(http.MethodPut).Headers(versionHeader, v1)
	router.HandleFunc("/target/{id:[0-9]+}", showTargetHandler(deps)).Methods(http.MethodGet).Headers(versionHeader, v1)
	router.HandleFunc("/target/{id:[0-9]+}", deleteTargetHandler(deps)).Methods(http.MethodDelete).Headers(versionHeader, v1)
