...
func InitRouter(deps Dependencies) (router *mux.Router) {
	...
	router.HandleFunc("/users/{username}/validate/{deck:[A-B]}", validateUserHandler(deps)).Methods(http.MethodPost, http.MethodOptions).Headers(versionHeader, v1)
	...
	router.HandleFunc("/processes/{id}", authenticate(updateProcessHandler(deps), deps)).Methods(http.MethodPut)
