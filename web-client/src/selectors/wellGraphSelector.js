...
import {
  getWells,
  getWellsPosition,
  getFilledWellsPosition,
  getFilledWellTargets,
} from "./wellSelectors";
  ...
  (wellGraphReducer, experimentTargets, wells) => {
    ...
    wellGraphData = wellGraphData.filter((ele) => {
      ...
      if (filledPositions.includes(ele.get("well_position"))) {
        ...
        const wellTargets = getFilledWellTargets(
          wells,
          ele.get("well_position")
        );
    ...
    if (selectedPositions.size !== 0) {
      wellGraphData = wellGraphData.filter((ele) =>
        selectedPositions.includes(ele.get("well_position"))
      );
    }
    // filtering active wells
    const activeTargets = graphTargets.filter(
      (ele) => ele.get("isActive") === true
    );
    ...
    if (activeTargets.size !== graphTargets.size) {
      ...
      wellGraphData = wellGraphData.filter((ele) =>
        activeTargetsIds.includes(ele.get("target_id"))
      );
    }
    ...
    const thresholdArray = activeTargets.map((ele) =>
      getThresholdLine(
        ele.get("target_name"),
        ele.get("threshold"),
        totalCycles,
